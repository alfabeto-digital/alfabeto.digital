  ...
  flake.nixosModules.caddy = { config, lib, pkgs, cfg, ... }: {
    ...
    services.caddy = {
      ...
      virtualHosts = {
        ...
        # Localhost-only HTTP endpoints for the Cloudflare tunnel.
        ...
        # cloudflared; these plain-HTTP blocks on a separate port avoid that.
        # Each subdomain gets its own vhost so Caddy matches by Host header.
        ...
        "http://mail.${cfg.domain}:${toString cfg.domain_tunnel_port}" = {
          extraConfig = ''
            bind 127.0.0.1
        ...
        "http://ntfy.${cfg.domain}:${toString cfg.domain_tunnel_port}" = {
          extraConfig = ''
            bind 127.0.0.1
            forward_auth localhost:${toString cfg.authelia_port} {
              uri /api/authz/forward-auth
        ...
        "http://adguard.${cfg.domain}:${toString cfg.domain_tunnel_port}" = {
          extraConfig = ''
            bind 127.0.0.1
            forward_auth localhost:${toString cfg.authelia_port} {
              uri /api/authz/forward-auth
